...
# (shared by both palm-mode and auto-routed)
# ─────────────────────────────────────────
...
def run_palm_pipeline(cap, actual_fps,
    ...
    Used in two cases:
        1. User selected palm mode directly (palm_mode=True)
        2. Auto mode routed to palm after face failed

    Input:
        cap          — VideoCapture from Step 1
        actual_fps   — fps from Step 1
        face_state   — FaceROIState (None if palm mode)
        face_profile — face SubjectProfile (None if palm mode)
        palm_mode    — True if user selected palm directly
