...
def measurement_failed(cap, reason, suggestions=None,
    ...
    Input:
        ...
        logger      — SessionLogger to write before exit
    """
    print(f"\n{'='*50}")
...
def print_rmssd_lines(hr_results, snr_report):
    ...
    if hr_results.get('hrv_available', False):
        ...
        msg = hr_results.get('hrv_fps_message', 'Requires 60fps+ camera')
        if not msg.endswith(')'):
            msg += ')'
        print(f"HRV:           {msg}")
...
def print_hr_line(hr_results):
    """Print Heart Rate line with reliability flag."""
    hr_flag = ""
...
def run_palm_pipeline(cap, actual_fps,
    ...
    # Log actual fps from palm recording
    if logger is not None:
...
if __name__ == "__main__":
    ...
    logger.set_fps(actual_fps)   # Step 1 fps estimate

    # ── Palm mode — skip face entirely ────────────
    if palm_mode:
        ...
        # Log routing (palm-direct — no face involved)
        logger.set_routing(
            route_palm = None,
    ...
    # ── Auto mode — face first ─────────────────────
        ...
        # Overwrite with actual measured fps
        logger.set_fps(fps_measured)
        ...
        if route_palm:
            if snr_report.get('std_floor_triggered'):
                routing_reason = 'std_floor'
            else:
                routing_reason = 'snr_score'
        ...
        # ── Face accepted — write log and finish ───
        if not route_palm:
            logger.set_final("face", hr_results, rr_bpm)
            logger.write()

        # ── Auto-route to palm if needed ───────────
        else:
