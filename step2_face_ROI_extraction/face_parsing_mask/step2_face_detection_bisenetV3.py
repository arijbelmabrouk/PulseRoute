...
# ── Parameters ────────────────────────────
# Every 15 frames — balance between fps and tracking
EVAL_EVERY_N_FRAMES = 15

# 256x256 input — ~2x faster than 512x512
# sufficient for clean region boundaries
...
def parse_face(model, frame_rgb, device):
    ...
    Uses reduced 256x256 input for ~2x speedup.
    ...
    # Reduced resolution — faster inference
    img = cv2.resize(
        frame_rgb,
...
def build_face_roi_masks(parsing, frame_h, frame_w):
    ...
    All boundaries derived from actual detected
    anatomical positions — fully adaptive:

        Forehead bottom → eyebrow top minus 15% gap
                          (was 2% — too tight, caused
                          missing forehead regions)
        Cheek top       → just below detected eyes
        Cheek bottom    → 70% of skin height
        Nose center     → actual detected nose x-position
        Nose strip      → 6% of face width
                          (was 12% — too wide, cheeks
                          too far from nose center)
    ...
    # ── Forehead boundary ──────────────────────────
    # Bottom = eyebrow top minus 15% buffer
    # FIX: was 0.02 (2%) — too tight, cut off most
    # of the forehead leaving only a thin strip.
    # 0.15 (15%) gives proper forehead coverage.
    ...
    # ── Nose center strip ──────────────────────────
    # FIX: reduced from 12% to 6% of face width
    # so cheeks sit closer to the nose center
    ...
    if len(skin_cols_all) > 0:
        ...
        # 6% of face width — closer to nose center
...
def run_face_detection():
    ...
    synchronously every EVAL_EVERY_N_FRAMES frames.

    fps optimizations:
        1. BiSeNet input reduced to 256x256
        2. Parse every 15 frames, cache between

    Forehead and cheek ROIs with fully adaptive
    ...
    while True:
        ...
        if run_parsing:
            ...
            # Only update mask if change is significant
            # Small BiSeNet variance gets ignored
            # Large changes (face moved) get applied
            should_update = True
            if cached_combined_mask is not None:
                # Calculate overlap between old and new mask
                intersection = np.sum(
                ...
                if union > 0:
                    ...
                    # IoU > 0.85 means masks are very similar
                    # keep old stable mask — ignore small variance
                    # IoU < 0.85 means face moved significantly
                    # update to new mask
                    should_update = iou < 0.85
            ...
            if should_update:
                ...
                cached_fitzpatrick, cached_ita = \
                    estimate_skin_tone(frame, new_combined)
