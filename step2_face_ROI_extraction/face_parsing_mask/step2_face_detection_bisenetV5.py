...
MODEL_PATH = os.path.join(CURRENT_DIR, "models", "bisenet_resnet18.pth")
...
def load_bisenet(model_path, device):
    """
    Load BiSeNet using the official model.py
    ...
    model.to(device)
    model.eval()
    print(f"BiSeNet loaded successfully from {model_path}")
    return model
...
def parse_face(model, frame_rgb, device):
    """
    Run BiSeNet inference on a single RGB frame.
...
def build_face_roi_masks(parsing, frame_h, frame_w):
    ...
    Fully adaptive to any face — boundaries derived
    from actual detected anatomical positions:

        Forehead top    → top of detected skin region
        Forehead bottom → just above detected eyebrows
        Cheek top       → just below detected eyes
        Cheek bottom    → 70% of skin height (above jaw)
        Cheek center    → actual detected nose x-position

    No hardcoded pixel values — all boundaries adapt
    to face size, position, and individual proportions.
    ...
    # ── Adaptive forehead boundary ─────────────────
    # Bottom of forehead = just above eyebrows
    # Use actual detected eyebrow position from parsing
    brow_pixels = np.where(
        (parsing == LABEL_L_BROW) |
        (parsing == LABEL_R_BROW)
    )
    if len(brow_pixels[0]) > 0:
        # Forehead ends just above highest eyebrow pixel
        ...
        # Fallback — top 30% of skin if no brows detected
        forehead_cut = skin_top + int(skin_height * 0.30)

    # Forehead mask — from skin top to just above brows
    forehead_mask = skin_mask.copy()
    forehead_mask[forehead_cut:, :] = 0

    # ── Adaptive cheek boundary ────────────────────
    # Top of cheeks = just below eyes
    # Use actual detected eye position from parsing
    eye_pixels = np.where(
        (parsing == LABEL_L_EYE) |
        (parsing == LABEL_R_EYE)
    )
    if len(eye_pixels[0]) > 0:
        # Cheeks start just below lowest eye pixel
        eye_bottom = int(np.max(eye_pixels[0]))
        cheek_top  = eye_bottom + int(skin_height * 0.04)
    else:
        # Fallback — 45% of skin height if no eyes detected
        cheek_top = skin_top + int(skin_height * 0.45)

    # Bottom of cheeks — above jaw/mouth
    # 70% of skin height is consistent across face types
    cheek_bottom = skin_top + int(skin_height * 0.70)

    # Safety check — ensure cheek region has height
    ...
    # ── Adaptive nose center strip ─────────────────
    # Use actual detected nose x-position from parsing
    nose_pixels = np.where(parsing == LABEL_NOSE)
    if len(nose_pixels[1]) > 0:
        nose_center_x = int(np.mean(nose_pixels[1]))
    else:
        # Fallback — horizontal center of skin region
        skin_cols     = np.where(skin_mask.sum(axis=0) > 0)[0]
        nose_center_x = int(np.mean(skin_cols)) \
                        if len(skin_cols) > 0 \
                        else frame_w // 2

    # Strip width — proportional to face width
    skin_cols_all = np.where(skin_mask.sum(axis=0) > 0)[0]
    if len(skin_cols_all) > 0:
        face_width  = int(skin_cols_all[-1] - skin_cols_all[0])
        nose_strip  = max(int(face_width * 0.12), 20)
    else:
        nose_strip  = frame_w // 10

    cheek_mask[
        :,
        nose_center_x - nose_strip: nose_center_x + nose_strip
    ] = 0
...
def visualize_parsing(parsing, frame_h, frame_w):
    """
    Colored visualization of all parsed regions.
    Press V while running to toggle this window.
...
def calculate_roi_stability(roi_area_history):
    """
    Measure ROI stability via coefficient of
    variation of ROI pixel area across frames.

    Input:
        roi_area_history — deque of pixel counts
...
def estimate_skin_tone(frame, mask):
    """
    Fitzpatrick type estimate using ITA formula.
    ...
    FST V-VI triggers palm fallback.
...
def calculate_motion(current_parsing, previous_parsing):
    """
    Estimate face motion via skin centroid
    displacement between frames.

    Input:
        current_parsing  — current label map
        previous_parsing — previous label map

    Output:
        motion_pixels — displacement in pixels
...
def confidence_from_mask(mask):
    """
    Estimate confidence from ROI pixel coverage.

    Input:
        mask — binary combined ROI mask
...
def run_face_detection():
    """
    Opens camera, loads BiSeNet from official
    zllrunning/face-parsing.PyTorch architecture,
    runs face parsing in real time.

    Extracts forehead (green) and cheek (blue) ROIs
    from pixel-level semantic segmentation.

    BiSeNet runs every EVAL_EVERY_N_FRAMES frames.
    Mask and evaluations cached between updates.
    ...
    print("Running — press Q to quit, V to toggle parsing view")
    ...
    while True:
        ...
        if run_parsing:
            ...
            forehead_mask, cheek_mask, combined_mask = \
                build_face_roi_masks(parsing, frame_h, frame_w)

            previous_parsing       = cached_parsing
            cached_parsing         = parsing
            cached_forehead_mask   = forehead_mask
            cached_cheek_mask      = cheek_mask
            cached_combined_mask   = combined_mask
        ...
        # Parsing visualization window
        if show_parsing_vis and cached_parsing is not None:
