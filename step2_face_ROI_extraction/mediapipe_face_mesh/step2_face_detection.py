...
# Landmark IDs for face ROI sub-regions
# Based on rPPG literature — verify visually on your face
# Forehead : best signal    — flat, good perfusion, no hair
# Cheeks   : good signal    — large area, good perfusion
# Nose     : medium signal  — good perfusion, specular reflection risk

FOREHEAD_LANDMARKS    = [10, 338, 297, 332, 284, 251,
                         109, 67,  103, 54,  21]

LEFT_CHEEK_LANDMARKS  = [116, 123, 147, 187, 207,
                         206, 203, 36,  31,  228]

RIGHT_CHEEK_LANDMARKS = [345, 352, 376, 411, 427,
                         426, 423, 266, 261, 448]

NOSE_LANDMARKS        = [1, 2, 5, 4, 195, 197]
...
def draw_key_landmarks(frame, landmarks, frame_w, frame_h):
    """
    Draw every 10th landmark ID on the face to avoid
    screen clutter — 468 landmarks total so showing
    every 10th gives ~47 visible IDs for verification.

    Also highlights the specific landmarks used in
    each ROI region in a different color so you can
    verify they land on the correct anatomical areas.
    ...
    Input:
        ...
        frame_h    — frame height
    """
    # All ROI landmark IDs combined for highlighting
    roi_ids = set(
        FOREHEAD_LANDMARKS +
    ...
    for idx, lm in enumerate(landmarks):
        ...
        if idx in roi_ids:
            ...
            # Every 10th non-ROI landmark — small white dot with ID
...
def is_face_visible(landmarks, frame_w, frame_h):
    ...
    Verifies that key ROI landmarks are within frame
    bounds and face is roughly frontal (not too tilted).

    Frontality check: nose tip (landmark 1) should be
    roughly centered between left and right face edges
    (landmarks 234 and 454).
    ...
    Output:
        ...
        frontality    — 0.0 to 1.0 (1.0 = perfectly frontal)
        visibility_pct — fraction of key landmarks in bounds
    """
    # Check key landmarks in bounds
    key_ids = FOREHEAD_LANDMARKS + LEFT_CHEEK_LANDMARKS + \
              RIGHT_CHEEK_LANDMARKS + NOSE_LANDMARKS
    ...
    # Frontality check — nose should be centered
    ...
    if face_width > 0:
        ...
        # Perfect center = 0.5, allow 0.2 tolerance either side
...
def calculate_face_confidence(face_results, frame_w, frame_h):
    ...
    Three signals:
        Score 1 — face visibility + frontality  weight 0.5
        Score 2 — key landmarks in bounds       weight 0.3
        Score 3 — landmark visibility mean      weight 0.2

    Note: Face Mesh DOES populate landmark visibility
    unlike Hands — so we can use it here.
    ...
    # Score 1 — face visibility and frontality
    ...
    # Score 2 — key landmarks in bounds
    ...
    # Score 3 — landmark visibility values
    # Face Mesh populates this reliably unlike Hands
    sample_ids  = [10, 234, 454, 1, 116, 345]
    visibilities = [landmarks[idx].visibility for idx in sample_ids]
...
def estimate_skin_tone(frame, mask):
    ...
    FST V-VI triggers palm fallback in the full pipeline.
...
def calculate_motion(current_landmarks,
    ...
    Uses forehead and nose landmarks as motion anchors
    — these are most sensitive to head movement.
    ...
    # Key anchors — forehead top, nose, cheek edges
...
def run_face_detection():
    ...
    Highlights four face ROI regions:
        Green  — Forehead   (best rPPG signal)
        Blue   — Left cheek (good signal)
        Blue   — Right cheek(good signal)
        Yellow — Nose       (medium signal)

    All 468 landmarks drawn — every 10th ID shown,
    ROI landmark IDs highlighted in cyan for verification.
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                # Draw landmarks with IDs for verification
                frame = draw_key_landmarks(
                    frame, landmarks, frame_w, frame_h
                )

                # Test 4 — motion every frame
                ...
                # Get pixel coordinates for each region
                ...
                # Tests 2 and 3 — every 10 frames only
                ...
                # Frontality indicator top right
                cv2.putText(
                    frame,
