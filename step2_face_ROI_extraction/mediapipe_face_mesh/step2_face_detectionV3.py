...
# Forehead threshold — verified from user's face
# forehead top at y=0.25, eyebrow level at ~0.42
FOREHEAD_Y_THRESHOLD = 0.42

# Cheek landmarks — 3 non-collinear points per side
# spread vertically: high (eye level), center, low (jaw)
# guarantees proper triangle with real area
LEFT_CHEEK_LANDMARKS  = [323, 426, 288]
RIGHT_CHEEK_LANDMARKS = [93,  206, 58]
...
# ─────────────────────────────────────────
# Geometry-based ROI builders
# ─────────────────────────────────────────

def get_forehead_region(landmarks, w, h):
    """
    Build forehead ROI using geometry.

    Selects all landmarks above eyebrow line
    (normalized y < FOREHEAD_Y_THRESHOLD) and
    builds a convex hull.

    Threshold verified on user face:
        forehead top  = y 0.25
        eyebrow level = y ~0.42
        nose          = y 0.55
    ...
    Output:
        list of (x, y) convex hull vertices
        empty list if insufficient points
    """
    pts = []
    for lm in landmarks:
        if lm.y < FOREHEAD_Y_THRESHOLD:
            x = int(lm.x * w)
            y = int(lm.y * h)
            pts.append((x, y))
...
def build_convex_region(pixel_points):
    """
    Build convex hull from pixel coordinates.
    Guarantees clean fillable polygon with no holes.
...
def expand_polygon(points, scale=1.3):
    """
    Expand polygon outward from centroid.
    More pixels = lower noise in extracted signal.

    Input:
        points — list of (x, y) polygon vertices
        scale  — expansion factor (1.3 = 30% larger)

    Output:
        list of expanded (x, y) integer coordinates
    """
    if len(points) < 3:
        return points
...
def get_combined_mask(all_region_pts, frame_h, frame_w):
    ...
    Always applies convex hull before fill —
    no holes from non-convex or badly ordered points.
...
def draw_roi(frame, points, color, alpha=0.3, label=None):
    ...
    Applies convex hull before drawing.
...
def draw_key_landmarks(frame, landmarks, frame_w, frame_h):
    """
    Draw every 10th landmark to avoid clutter.
    Cheek ROI landmarks shown in cyan.
    Forehead zone boundary shown as horizontal line.
    ...
    Input:
        ...
        frame_h    — frame height
    """
    roi_ids = set(LEFT_CHEEK_LANDMARKS + RIGHT_CHEEK_LANDMARKS)

    # Draw forehead threshold line for visual reference
    threshold_y = int(FOREHEAD_Y_THRESHOLD * frame_h)
    cv2.line(frame,
             (0, threshold_y),
             (frame_w, threshold_y),
             (100, 100, 100), 1)
    ...
    for idx, lm in enumerate(landmarks):
        ...
        if idx in roi_ids:
            # Cheek ROI landmarks — cyan dot with ID
...
def is_face_visible(landmarks, frame_w, frame_h):
    ...
    """
    key_ids = LEFT_CHEEK_LANDMARKS + RIGHT_CHEEK_LANDMARKS
...
def run_face_detection():
    ...
    Two ROI regions:
        Green — Forehead  (geometry-based, y threshold verified)
        Blue  — Cheeks    (3 non-collinear points + hull + expand)

    Forehead threshold line drawn on screen for verification.
    Cheek anchor landmarks shown as cyan dots with IDs.
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                # Draw landmark IDs + threshold line
                ...
                # Forehead — geometry based
                # all landmarks above verified threshold
                forehead_pts = get_forehead_region(
                    landmarks, frame_w, frame_h
                )

                # Cheeks — 3 non-collinear verified points
                # spread high/center/low for real triangle area
                left_raw  = get_landmark_pixels(
                    landmarks, LEFT_CHEEK_LANDMARKS,
                    frame_w, frame_h
                )
                right_raw = get_landmark_pixels(
                    landmarks, RIGHT_CHEEK_LANDMARKS,
                    frame_w, frame_h
                )

                # Convex hull then expand for more pixels
                left_cheek_pts  = expand_polygon(
                    build_convex_region(left_raw), scale=1.3
                )
                right_cheek_pts = expand_polygon(
                    build_convex_region(right_raw), scale=1.3
                )
