...
def get_combined_mask(all_region_pts, frame_h, frame_w):
    ...
    Reduces three NumPy array allocations to one —
    recovers fps vs separate mask approach.
...
def is_palm_facing_camera(landmarks):
    ...
    Verified on user setup: cross > 0 = palm facing.
...
def run_palm_detection():
    ...
    10 frames only — cached between evaluations to
    preserve fps.

    Single combined mask replaces three separate masks
    for improved fps performance.
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                if palm_facing:
                    ...
                    # Fix 1 — single combined mask
                    # replaces three separate masks + clip merge
                    ...
                    # Fix 2 — skin tone and lighting
                    # every 10 frames only, cached between updates
