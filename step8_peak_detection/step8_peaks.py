...
def reject_harmonics(candidate_freqs,
    ...
    for i in range(len(freqs_s)):
        ...
        for j in range(i + 1, len(freqs_s)):
            ...
            # j is 2nd or 3rd harmonic of i
            if abs(ratio - 2.0) < HARMONIC_TOLERANCE \
            or abs(ratio - 3.0) < HARMONIC_TOLERANCE:
                is_harmonic[j] = True

            # j is sub-harmonic of i
            # (i.e. i is 2x j — j should be kept,
            #  but check if j was already processed)
        for j in range(i):
...
def find_dominant_peak(freqs, power):
    ...
    Steps:
        1. Isolate valid HR band (40-180 BPM)
...
def filter_rr_intervals(rr_intervals):
    ...
    Keeps only intervals within 30% of the median.
    This removes noise peaks that created artificially
    short or long intervals while preserving real
    HRV variation which is typically < 20% of mean.

    Works for all users:
        Athlete at rest: RR ~1200ms, keeps 840-1560ms
        Normal resting:  RR ~857ms,  keeps 600-1114ms
        Running:         RR ~400ms,  keeps 280-520ms

    Input:
        rr_intervals — raw RR intervals in ms

    Output:
        filtered_rr — physiologically valid intervals
    """
    if len(rr_intervals) < 3:
        return rr_intervals

    median_rr = np.median(rr_intervals)
    valid     = np.abs(rr_intervals - median_rr) \
                < 0.30 * median_rr

    return rr_intervals[valid]


def detect_beat_peaks(filtered, fps, hr_bpm_fft=None):
    """
    Detect individual heartbeat peaks in time domain.

    Uses two-stage approach:
        Stage 1 — FFT-guided period detection
                  if hr_bpm_fft provided
                  finds peak in each expected period
                  robust for smooth rPPG waveforms

        Stage 2 — Prominence-based detection
                  fallback if FFT period not available
                  or as cross-validation

    Applies RR consistency filter to remove outliers.

    Input:
        filtered    — filtered pulse waveform (Step 6)
        fps         — actual fps
        hr_bpm_fft  — HR from FFT (Step 7) optional
                      if provided uses period guidance
    ...
    if hr_bpm_fft is not None and \
       HR_MIN_BPM <= hr_bpm_fft <= HR_MAX_BPM:
        ...
        # Handle last partial window if > 50% of period
        remaining = n - start
...
def assess_peak_quality(hr_bpm, peak_hz,
    ...
    Three checks:
        ...
        2. Sufficient beats for HRV (at least 10)
